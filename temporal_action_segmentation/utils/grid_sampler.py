...
Specifically, it samples N frames from a video with N frames according to a truncated normal distribution.
Therefore, it can be regarded as local acceleration or deceleration within a video.
It can serve as a strong data augmentation for action segnemtation task by setting the 'if_warp=True' in batch_gen.BatchGenerator.next_batch.
We do not use this trick in our paper, but it does give better results :).
"""
...
class TimeWarpLayer(nn.Module):
    ...
    def forward(self, x, grid, mode="bilinear"):
        ...
        out = TF.grid_sample(input=x.view(x_4dviews), grid=grid.view(grid_4dviews), mode=mode, align_corners=True).view(
            x.shape
        )
        return out
...
class GridSampler:
    ...
    def sample(self, batchsize=1):
        ...
        grid = np.sort(
            np.random.choice(vals.reshape(-1), size=self.N_primary, replace=False)
        )  # pick one center for each primary
