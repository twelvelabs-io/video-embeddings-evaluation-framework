...
from utils.grid_sampler import GridSampler, TimeWarpLayer
...
class BatchGenerator(object):
    ...
    def read_data(self, vid_list_file):
        ...
        self.list_of_examples = [
            ...
            if os.path.exists(os.path.join(self.features_path, vid.split(".")[0] + "_c.npy"))
        ]
        self.gts = [os.path.join(self.gt_path, vid.split(".")[0] + ".txt") for vid in self.list_of_examples]
        self.features = [
            os.path.join(self.features_path, vid.split(".")[0] + "_c.npy") for vid in self.list_of_examples
        ]
        self.my_shuffle()
    ...
    def merge(self, bg, suffix):
        ...
        logging.info("Merge! Dataset length:{}".format(len(self.list_of_examples)))
    ...
    def next_batch(
        ...
        for idx, vid in enumerate(batch):
            features = np.load(batch_features[idx]).transpose(1, 0)
            file_ptr = open(batch_gts[idx], "r")
            content = file_ptr.read().split("\n")[:-1]
            classes = np.asarray([self.actions_dict[c] for c in content])
            if self.sample_rate > 1:
                classes = classes[:: self.sample_rate]
            if len(classes) != features.shape[1]:
                features = (
                    F.interpolate(torch.from_numpy(features).unsqueeze(0).float(), size=classes.shape[0], mode="linear")
                    .squeeze(0)
                    .numpy()
                )
        ...
        batch_input_tensor = torch.zeros(
            len(batch_input), np.shape(batch_input[0])[0], max(length_of_sequences), dtype=torch.float
        )  # bs, C_in, L_in
        ...
        for i in range(len(batch_input)):
            if if_warp:
                warped_input, warped_target = self.warp_video(
                    torch.from_numpy(batch_input[i]).unsqueeze(0), torch.from_numpy(batch_target[i]).unsqueeze(0)
                )
                (
...
class SegDataset(Dataset):
    ...
    def __len__(self):
        return len(self.bg.list_of_examples)

    def __getitem__(self, idx):
        vid = self.bg.list_of_examples[idx]
        features = np.load(self.bg.features[idx]).transpose(1, 0)
        file_ptr = open(self.bg.gts[idx], "r")
        content = file_ptr.read().split("\n")[:-1]
        classes = np.asarray([self.bg.actions_dict[c] for c in content])
        if self.bg.sample_rate > 1:
            classes = classes[:: self.bg.sample_rate]
        if len(classes) != features.shape[1]:
            features = (
                F.interpolate(torch.from_numpy(features).unsqueeze(0).float(), size=classes.shape[0], mode="linear")
                .squeeze(0)
        ...
        return features, classes, vid

    def collate_fn(self, batch):
        batch_input = [item[0] for item in batch]
        batch_target = [item[1] for item in batch]
        batch_vid = [item[2] for item in batch]
        length_of_sequences = list(map(len, batch_target))
        batch_input_tensor = torch.zeros(
            len(batch_input), np.shape(batch_input[0])[0], max(length_of_sequences), dtype=torch.float
        )
        batch_target_tensor = torch.ones(len(batch_input), max(length_of_sequences), dtype=torch.long) * (-100)
        mask = torch.zeros(len(batch_input), self.bg.num_classes, max(length_of_sequences), dtype=torch.float)
        for i in range(len(batch_input)):
            batch_input_tensor[i, :, : np.shape(batch_input[i])[1]] = torch.from_numpy(batch_input[i])
            batch_target_tensor[i, : np.shape(batch_target[i])[0]] = torch.from_numpy(batch_target[i])
            mask[i, :, : np.shape(batch_target[i])[0]] = torch.ones(self.bg.num_classes, np.shape(batch_target[i])[0])
