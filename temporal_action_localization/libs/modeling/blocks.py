...
class MaskedConv1D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if self.stride > 1:
            ...
            out_mask = F.interpolate(mask.to(x.dtype), size=T // self.stride, mode="nearest")
        else:
...
class MaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        self.query_conv = MaskedConv1D(
            self.n_embd, self.n_embd, kernel_size, stride=stride, padding=padding, groups=self.n_embd, bias=False
        )
        # layernorm
...
class MaxPooler(nn.Module):
    ...
    def forward(self, x, mask, **kwargs):
        ...
        if self.stride > 1:
            # downsample the mask using nearest neighbor
            out_mask = F.interpolate(mask.to(x.dtype), size=x.size(-1) // self.stride, mode="nearest")
        else:
            # masking out the features
            out_mask = mask
