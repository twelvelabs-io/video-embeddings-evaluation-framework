    ...
    def __init__(
        ...
        for idx in range(num_layers - 1):
            if idx == 0:
                ...
                out_dim = feat_dim
            self.head.append(
                MaskedConv1D(in_dim, out_dim, kernel_size, stride=1, padding=kernel_size // 2, bias=(not with_ln))
            )
            if with_ln:
...
class PtTransformerRegHead(nn.Module):
    ...
    def __init__(self, input_dim, feat_dim, fpn_levels, num_layers=3, kernel_size=3, act_layer=nn.ReLU, with_ln=False):
        super().__init__()
...
class PtTransformer(nn.Module):
    ...
    def forward(self, video_list):
        ...
        if self.training:
            ...
            losses = self.losses(fpn_masks, out_cls_logits, out_offsets, gt_cls_labels, gt_offsets)
            return losses
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        min_len_mask = torch.logical_and((lens <= (min_len[:, None] + 1e-3)), (lens < float("inf"))).to(
            reg_targets.dtype
        )
    ...
    def losses(self, fpn_masks, out_cls_logits, out_offsets, gt_cls_labels, gt_offsets):
        ...
        return {"cls_loss": cls_loss, "reg_loss": reg_loss, "final_loss": final_loss}

    @torch.no_grad()
    def inference(self, video_list, points, fpn_masks, out_cls_logits, out_offsets):
    ...
    def postprocessing(self, results):
        ...
        for results_per_vid in results:
            ...
            processed_results.append({"video_id": vidx, "segments": segs, "scores": scores, "labels": labels})
