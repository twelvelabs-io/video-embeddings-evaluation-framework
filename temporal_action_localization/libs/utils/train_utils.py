...
def make_optimizer(model, optimizer_config):
    ...
    if optimizer_config["type"] == "SGD":
        optimizer = optim.SGD(optim_groups, lr=optimizer_config["learning_rate"], momentum=optimizer_config["momentum"])
    ...
    if optimizer_config["warmup"]:
        ...
        if optimizer_config["schedule_type"] == "cosine":
            ...
            scheduler = LinearWarmupMultiStepLR(
                optimizer, warmup_steps, steps, gamma=optimizer_config["schedule_gamma"], last_epoch=last_epoch
            )
        else:
...
def train_one_epoch(
    ...
    # main training loop
    logging.info("[Train]: Epoch {:d} started".format(curr_epoch))
    start = time.time()
...
def valid_one_epoch(
    val_loader, model, curr_epoch, ext_score_file=None, evaluator=None, output_file=None, tb_writer=None, print_freq=20
):
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        if (iter_idx != 0) and iter_idx % (print_freq) == 0:
            ...
            # print timing
            logging.info(
                "Test: [{0:05d}/{1:05d}]\t" "Time {batch_time.val:.2f} ({batch_time.avg:.2f})".format(
